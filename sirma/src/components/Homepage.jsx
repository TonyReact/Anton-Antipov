...
// group Employees by project id , change date string to JS newDate
  ...
  const Proj_Emps = arr.reduce( (r,[EmployeeID, ProjectID, StartDate, EndDate])=>
    {
  ...
  let Result  =  
   ...
   const em2 = Result.map(el =>  Array.from(el.emB))
	...
	const det = Result.map(el => Array.from(JSON.stringify(Object.values(el.details[0])).replaceAll('[','').replaceAll(']','').replaceAll(',','-')))
