...
const Homepage = () => {
  ...
  function showResult(formatedData) {
    ...
    const Proj_Emps = formatedData.reduce(
    ...
    for (let proj in Proj_Emps)
      for (let i = 0; i < Proj_Emps[proj].length - 1; i++)
        for (let j = i + 1; j < Proj_Emps[proj].length; j++) {
          ...
          if (
            (employeeA.endDay <= employeeB.endDay &&
              employeeA.endDay > employeeB.startDay) ||
            (employeeB.endDay <= employeeA.endDay &&
              employeeB.endDay > employeeA.startDay)
          ) {
            const firstDate =
                employeeA.startDay > employeeB.startDay
                  ? employeeA.startDay
                  : employeeB.startDay,
              secondDate =
                employeeA.endDay < employeeB.endDay
